  ...
  setHeight() {
    requestAnimationFrame(() => {
      ...
      if (this.classList.contains('header--center')) {
        document.documentElement.style.setProperty('--header-nav-height', Math.round(this.getElementById('MenuToggle').clientHeight) + 'px');
      }
    });
  }
}
...
class MarqueeElement extends HTMLElement {
  ...
  get speed() {
    return parseInt(this.dataset.speed || 16);
  }

  get maximum() {
    return parseInt(this.dataset.maximum || Math.ceil(this.parentWidth / this.childElementWidth) + 2);
  }
...
class VideoMedia extends DeferredMedia {
  constructor() {
    super();
     ...
     this.progress = this.querySelector('.video__progress') || null;
  }
...
class MediaGallery extends HTMLElement {
  ...
  openZoom(index = 0) {
    let dataSource = this.sliderGallery.itemsToShow.map((media) => {
      ...
      if (media.dataset.mediaType === 'video' || media.dataset.mediaType === 'external_video' || media.dataset.mediaType === 'model') {
